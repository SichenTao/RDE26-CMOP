...
**RDE26-CMOP は、IEEE WCCI 2026 / IEEE CEC 2026 の「精度と速度を考慮した数値最適化コンペティション」における Competition 4-CMOPs トラックの公式優勝・第 1 位のアルゴリズムです。**

本リポジトリは、制約付き多目的最適化トラック向けの最終版オープンソースアルゴリズムパッケージを提供します。
...
| トラック | Competition 4-CMOPs |
| 問題クラス | 制約付き多目的最適化 |
| ベンチマーク | スケーラブルな決定空間制約付き多目的最適化問題 15 問 |
| 実行回数 | 各問題につき 15 回の独立実行 |
...
- 🏆 CEC 2026 CMOPs トラックの公式優勝アルゴリズムです。
...
| `code/rde26-cmop/main.py` | トラック専用の実行エントリ |
| `code/rde26-cmop/rde_cmop_native/` | 最終アルゴリズムのソースコード |
...
生成された出力は `code/rde26-cmop/outputs/` に書き込まれます。
